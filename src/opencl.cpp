...
void enqueue_kernel(cl_kernel kernel,cl_uint work_dim ,const size_t* global_work_size, size_t* offset, const size_t* local_work_size) {
    cl_int err = clEnqueueNDRangeKernel(_global_queue, kernel, work_dim, offset, global_work_size, local_work_size, 0, nullptr, nullptr);
    if (err != CL_SUCCESS) {
...
cl_mem create_buffer(const buffer_type buffer_type, const size_t size, void* host_ptr) {
    ...
    switch (buffer_type) {
        case buffer_type::READ_ONLY:
            buffer = clCreateBuffer(_global_context, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, size, host_ptr, &err);
            break;
        case buffer_type::WRITE_ONLY:
            buffer = clCreateBuffer(_global_context, CL_MEM_WRITE_ONLY | CL_MEM_USE_HOST_PTR, size, host_ptr, &err);
            break;
        case buffer_type::READ_WRITE:
            buffer = clCreateBuffer(_global_context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, size, host_ptr, &err);
            break;
        default:
    ...
    if (err != CL_SUCCESS) {
        LOG(ERROR) << "failed to create buffer. err: " << err << std::endl;
        exit(EXIT_FAILURE);
    }
